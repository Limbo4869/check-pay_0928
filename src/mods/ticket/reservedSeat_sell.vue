<template>
  <div>
    <yd-navbar title="留位售出查询" style="margin-bottom: 10px">
      <div slot="left" @click="goView('/ticket/ticket')" class="iconfont icon-back"></div>
      <!--<div slot="right" @click="goView('/bus/history')" class="el-icon-search"></div>-->
    </yd-navbar>
    <div>
      <el-row>
        <el-col :span="8">
          <div class="grid-content bg-purple-dark" style="margin: auto 5px">
          <!--<yd-search v-model="value1" :on-submit="submitHandler" placeholder="班次号"  cancel-text></yd-search>-->
          <el-input placeholder="班次号"  v-model="schfilter" clearable>
          </el-input>
        </div>
        </el-col>
        <el-col :span="8">
          <div class="grid-content bg-purple-dark" style="margin: auto 5px">
            <!--<yd-search v-model="value1" :on-submit="submitHandler" placeholder="班次号"  cancel-text></yd-search>-->
            <el-input placeholder="站点名称"  v-model="schfilter" clearable>
            </el-input>
          </div>
        </el-col>
        <el-col :span="8">
        <div class="grid-content bg-purple-dark" style="margin: auto 5px">
          <el-input placeholder="站码"  v-model="schfilter" clearable>
          </el-input>
        </div>
      </el-col>
      </el-row>
      <el-row>
        <el-col :span="6"><div class="grid-content bg-purple">
          <yd-cell-item>
                  <span slot="left" style="margin-left: 12px">
                     <yd-datetime type="day" style="height: 30px;line-height: 30px;float: left;" v-model="datetime0"></yd-datetime>
                  </span>
            <span slot="left" style="margin-left: 5px" class="el-icon-caret-bottom"></span>
          </yd-cell-item>
        </div></el-col>
        <el-col :span="6"><div class="grid-content bg-purple-light">
          <yd-cell-item>
                  <span slot="left" style="margin-left: 2px">
                     <yd-datetime type="day" style="height: 30px;line-height: 30px;float: left;" v-model="datetime1"></yd-datetime>
                  </span>
            <span slot="left" style="margin-left: 5px" class="el-icon-caret-bottom"></span>
          </yd-cell-item>
        </div></el-col>
        <el-col :span="6"><div class="grid-content bg-purple-light" style="margin-top: 12px">
          <el-radio v-model="radio" label="1">全部出票</el-radio>
        </div></el-col>
        <el-col :span="6"><div class="grid-content bg-purple-light" style="margin-top: 12px">
          <el-radio v-model="radio" label="2">未出票</el-radio>
        </div></el-col>
      </el-row>
    </div>
    <div style="margin-top: 20px">
      <el-table
        :data="tableData"
        height="550"
        highlight-current-row
        stripe
        style="width: 100%">
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              <el-form-item label="日期">
                <span>{{ props.row.RQ_tb }}</span>
              </el-form-item>
              <el-form-item label="班次号">
                <span>{{ props.row.BCH_tb }}</span>
              </el-form-item>
              <el-form-item label="发车">
                <span>{{ props.row.FCSJ_tb }}</span>
              </el-form-item>
              <el-form-item label="站点">
                <span>{{ props.row.ZD_tb }}</span>
              </el-form-item>
              <el-form-item label="留位数">
                <span>{{ props.row.LWS_tb }}</span>
              </el-form-item>
              <el-form-item label="出票数">
                <span>{{ props.row.CPS_tb  }}</span>
              </el-form-item>
              <el-form-item label="未出票">
                <span>{{ props.row.WCP_tb }}</span>
              </el-form-item>
              <el-form-item label="用户数">
                <span>{{ props.row.YHS_tb }}</span>
              </el-form-item>
            </el-form>
          </template>
        </el-table-column>
        <el-table-column
          label="日期"
          prop="RQ_tb">
        </el-table-column>
        <el-table-column
          label="班次号"
          prop="BCH_tb">
        </el-table-column>
        <el-table-column
          label="发车时间"
          prop="FCSJ_tb">
        </el-table-column>
      </el-table>
    </div>
  </div>
</template>
<script>
  export default {
    data() {
      return {
        tableData5: [{
          RQ_tb: '08-01',
          BCH_tb: '8001',
          FCSJ_tb: '18:30',
          ZD_tb: '天河站',
          LWS_tb: '10',
          CPS_tb: '5',
          WCP_tb: '5',
          YHS_tb:'10',
        },
          {
            RQ_tb: '08-04',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-12',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-16',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-08',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-06',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-27',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-21',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-17',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-11',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-06',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-19',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-09',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-29',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }, {
            RQ_tb: '08-19',
            BCH_tb: '8001',
            FCSJ_tb: '18:30',
            ZD_tb: '天河站',
            LWS_tb: '10',
            CPS_tb: '5',
            WCP_tb: '5',
            YHS_tb:'10',
          }
        ],
        radio:1,
        tableData:[],
        schfilter: '',
        value1: '',
        datetime0: '08-08',
        datetime1: '08-26',
      }
    },
    methods: {
      submitHandler(value) {
        this.$dialog.toast({mes: `工号：${value}`});
      },
      formatTooltip(val) {
        return val / 100;
      },
      dateFilter() {
        this.tableData = this.tableData.filter(item => {
          let year = new Date().getFullYear();
          let start = new Date(year + '-' + this.datetime0).getTime();
          let end = new Date(year + '-' + this.datetime1).getTime();
          let item_data = new Date(year + '-' + item.RQ_tb).getTime();
          return start <= item_data && item_data <= end;
        });
      },
      keywordFilter() {
        let val = this.schfilter;
        if (val ==''){
          this.tableData = this.tableData5;
        } else {
          this.tableData = this.tableData5.filter(item => (
            !item.GH_tb.indexOf(val)))
        }
      },
      tableFilter() {
        this.keywordFilter();
        this.dateFilter();
      }
    },
    watch: {
      schfilter(){
        this.tableFilter();
      },
      datetime0(){
        this.tableFilter();
      },
      datetime1(){
        this.tableFilter();
      }
    },
    created(){
      this.tableData = this.tableData5;
      this.tableFilter();
    }
  }

  // watch: {
  //   schfilter: function(val, oldVal){
  //     if(val==''){
  //       this.tableData=this.tableData5;
  //     }else{
  //       this.tableData = this.tableData5.filter( item => (!item.busnub_tb.indexOf(val)));
  //       console.log(val);
  //     }
  //   },
  // },
</script>



<style scoped>
  .grid-content bg-purple-light {
    margin-top: 10px!important;
  }
  .yd-cell-item {
    padding-left: 0 !important;
  }
  .yd-datetime-input {
    font-size: 12px;
  }
  .icon-arrow-down:before {
    font-size: 10px;
  }
  .demo-table-expand {
    font-size: 0;
  }
  .demo-table-expand label {
    width: 90px;
    color: #99a9bf;
  }
  .demo-table-expand .el-form-item {
    margin-right: 0;
    margin-bottom: 0;
    width: 50%;
  }
  body {
    font-family: Helvetica Neue, Arial, sans-serif;
    font-size: 14px;
    color: #444;
  }

</style>
